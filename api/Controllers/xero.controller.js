...
import open from "open";
import { v4 as uuidv4 } from "uuid";
// imports to use contacts and address update in userList.json
...
export const getContacts = async (req, res) => {
    try {
        ...
        // Send contacts data as response
        res.json(contacts);
    } catch (error) {
        res.status(500).json({ error: `An error occurred while fetching contacts : ${error.message}` });
    }
};
...
export const updateAddress = async (req, res) => {
    try {
        ...
        const contactIndex = contacts.findIndex(
            (c) =>
                c.name.trim().toLowerCase() === currentUser.name.trim().toLowerCase() &&
                (c.caseId === currentUser.caseId || c.caseId === currentUser.caseId)
        );
        ...
        // Update contact's addresses
        contacts[contactIndex].Addresses = [
            {
                AddressType: "STREET",
                AddressLine1: formData.AddressLine1,
                AddressLine2: formData.AddressLine2 || "",
        ...
        fs.writeFileSync(userListPath, JSON.stringify(contacts, null, 2));
